// @ts-nocheck
/**
 * Seed script — populates all Amplify DynamoDB tables with resume data.
 *
 * Run ONCE after `npx ampx sandbox` finishes deploying:
 *   npx ts-node --project tsconfig.seed.json scripts/seed.ts
 *
 * Safe to re-run: it will just create duplicate records (use the
 * Amplify Console to delete old ones if needed).
 */
...
async function seedProfile() {
  await client.models.Profile.create({
    ...
    photo_url: null, // upload to S3 and paste URL here
    is_active: true,
  });
...
async function seedInternships() {
  ...
  for (const i of internships) await client.models.Internship.create(i);
...
async function seedProjects() {
  const projects = [
    ...
    },
    {
      ...
      description:
        "AWS Kinesis Firehose streams stock data to S3; Azure Data Factory orchestrates ETL into Databricks (PySpark + Delta Lake). Advanced metrics (VWAP, price trends) via window functions; interactive Power BI dashboards.",
  ...
  for (const p of projects) await client.models.Project.create(p);
...
async function seedSkills() {
  const skills = [
    ...
    { category: "Backend & APIs", items: ["FastAPI", "Flask", ".NET Core", "ASP.NET Core Web API", "REST APIs", "Microservices", "SignalR (WebSockets)", "JWT Auth", "Redis"], subcategories: null, sort_order: 2, is_active: true },
    {
      ...
      items: null,
      subcategories: [
        { name: "Azure", items: ["Data Factory", "Functions", "App Services", "Service Bus", "Blob Storage", "Key Vaults", "DevOps (CI/CD)"] },
        { name: "AWS",   items: ["S3", "Lambda", "Kinesis Firehose", "ECR", "CodePipeline", "DynamoDB", "API Gateway", "CloudWatch", "Amplify"] },
        { name: "GCP",   items: ["BigQuery", "Google Kubernetes Engine", "Compute Engine", "Cloud Storage", "App Engine"] },
      ],
      sort_order: 4,
      is_active: true,
    },
  ...
  for (const s of skills) await client.models.Skill.create(s as Parameters<typeof client.models.Skill.create>[0]);
...
async function seedEducations() {
  ...
  for (const e of educations) await client.models.Education.create(e);
...
async function seedCertifications() {
  ...
  for (const c of certs) await client.models.Certification.create(c);
